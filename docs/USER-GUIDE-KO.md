...
- 다른 앱이 남긴 활동 기록(events)은 제목과 상세 내용이 함께 검색됩니다.
- 검색어가 없으면 최신 항목 20개를 표시합니다.

## 검색 기간 넓히기(Load 3 more months)

다른 앱이 남긴 활동 기록은 **기본적으로 최근 3개월치만** 불러옵니다. 기록 파일이 앱마다·기기마다·달마다 하나씩 생기기 때문에, 제한을 두지 않으면 새로고침이 느려집니다.

1. 설정(Settings) → 데이터(Data)를 엽니다.
2. `Load 3 more months`를 누릅니다. 3개월씩 넓어지고 곧바로 새로고침됩니다.
3. 안내 문구에 현재 범위가 표시됩니다. 최대 24개월까지 넓힐 수 있고, 끝까지 넓히면 버튼이 `All events loaded`로 바뀌며 비활성화됩니다.

넓힌 범위는 저장되어 다음에 앱을 열 때도 유지됩니다. Tide와 Clip 기록은 이 제한과 무관하게 전부 검색됩니다.

## 복사하고 원래 앱 열기

- 결과의 본문 영역을 누르면 전체 원문이 클립보드에 복사되고 `Copied` 토스트가 나타납니다.
- 오른쪽의 열기 버튼을 누르면 같은 탭에서 그 기록을 남긴 앱을 엽니다. 여는 주소가 없는 기록은 열기 버튼 없이 복사만 됩니다.
